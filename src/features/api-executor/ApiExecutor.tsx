...
export const ApiExecutor = ({ endpoint }: { endpoint?: RestfulEndpoint | null }) => {
    ...
    // Save params and headers to local storage when they change
    useEffect(() => {
        if (endpoint) {
            LocalStorageHelper.setParams(endpoint.url, updatedParams);
        }
    }, [updatedParams, endpoint]);

    useEffect(() => {
        if (endpoint) {
            LocalStorageHelper.setHeaders(endpoint.url, updatedHeaders);
        }
    }, [updatedHeaders, endpoint]);
