...
export const ParamsTab = ({
    ...
    if (restfulEndpointUrl) {
        if (tabType === TAB_TYPES.PARAMETERS) {
            storedParams = LocalStorageHelper.getParams(restfulEndpointUrl);
        } else if (tabType === TAB_TYPES.HEADERS) {
            storedParams = LocalStorageHelper.getHeaders(restfulEndpointUrl);
        }
    }

    const [rows, setRows] = useState<Row[]>([]);

    // Step 1: Load initial params or headers from local storage or use defaults
    useEffect(() => {
        if (restfulEndpointUrl) {
            const params = storedParams ? storedParams : {};

            const initialRows = Object.entries(params).map(([key, value]) => ({
                selected: true,
                key,
                value,
                description: "",
    ...
    // Step 2: Persist changes to local storage
    useEffect(() => {
        if (restfulEndpointUrl) {
            const params = rows
            ...
            if (tabType === TAB_TYPES.PARAMETERS) {
                ...
                LocalStorageHelper.setHeaders(restfulEndpointUrl, params);
            }
        }
    }, [rows, restfulEndpointUrl, tabType]);
